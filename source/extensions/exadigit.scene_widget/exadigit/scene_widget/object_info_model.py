...
class ObjInfoModel(sc.AbstractManipulatorModel):
    ...
    def get_custom_attributes(self):
        attributes_dict = {}
        if not self.prim:
            print("DEBUG: No prim is selected.")
            return attributes_dict

        print("DEBUG: Selected prim path:", self.prim.GetPath())

        # Look for a child prim called "attributes" under the selected prim.
        attr_scope = self.prim.GetChild("attributes")
        if not attr_scope or not attr_scope.IsValid():
            print("DEBUG: No valid 'attributes' scope found under prim", self.prim.GetPath())
            return attributes_dict

        print("DEBUG: Found attributes scope:", attr_scope.GetPath())

        for attr in attr_scope.GetAttributes():
            attr_name = attr.GetName()
            attr_value = attr.Get()

            # Exclude attributes we don't want to display
            if attr_name in ["purpose", "visibility"]:
                continue  # Skip these attributes

            print("DEBUG: Adding attribute:", attr_name, "with value:", attr_value)
